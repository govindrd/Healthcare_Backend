...
4. Select your `healthcare_backend` repository
...
heroku config:set \
  ...
  ALLOWED_HOSTS=healthcare-backend-app.herokuapp.com

# Add Procfile (create file in project root):
# web: gunicorn healthcare_backend.wsgi:application
...
docker tag healthcare-backend:latest \
  YOUR_AWS_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/healthcare-backend:latest
...
6. Set start command: `gunicorn healthcare_backend.wsgi:application`
...
- ✅ Models (Patient, Doctor, CustomUser, etc.)
